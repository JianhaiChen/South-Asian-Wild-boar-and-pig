...
#   offsets = {}
    ...
    # plot val
    for chrom, color in zip(df[chr_col].unique(), cycle(['#3B5488', '#53BBD5'])):
#       offsets[chrom] = loc_offset
        ...
        if isinstance(highlight, pd.DataFrame):
            ...
            if hdf.shape[0] > 0:
                ...
                hdf.plot(kind='scatter', x=loc_col, y=val_col, ax=ax, s=markersize, color='#FF0000', marker='o') #highlight
        if cutoff:
            ax.hlines(cutoff[chrom], tmpdf[loc_col].values[0], tmpdf[loc_col].values[-1], colors='#E40017', linestyles='dashed')
...
@click.option('--xlabel', default="Chromosome", help='输入散点图x轴标签的名称')
@click.option('--ylabel', default=r"$-log_{10}{(q)}$", help='输入散点图y轴标签的名称')
